...
class Poll extends React.Component {
  ...
  componentDidMount() {
    ...
    if (this.id.match(/^[0-9a-fA-F]{24}$/)) {
        // If id is valid, fetch poll data from server
        ...
        .then((response) => {
            this.setState({ 
                ...
                // Data is styled this way, in order to be passed to chart
  ...
  vote(pollId, optionId) {
    if (sessionStorage.getItem('userData')) {
        // Allow user to vote only if logged in
  ...
  handleOptionAdd(event) {
    if (this.state.newOption.length > 0) {
        ...
        .then((response) => {
            this.setState({
                // Data is styled this way, in order to get passed to chart
                data: [['Option', 'Number of Votes']].concat(response.data.options.map((option) => {
                    return [option.name, option.voters.length];
                })),
                optionIds: response.data.options.map((option) => {
                ...
                <div style={{ marginBottom: "2em" }}>
                    ...
                    <div className="iconBtns">
                        {sessionStorage.getItem('userData') && 
                            (JSON.parse(sessionStorage.getItem('userData')).local.username === this.state.username) &&
                    ...
                    <p style={style.addedBy}>
                      Added by: <Link to={'/user/' + this.state.username}>{this.state.username}</Link>
                    </p>
                </div>
