...
class NewPoll extends React.Component {
    ...
    handleInputChange(event) {
        ...
        if (name === 'question' && this.state.question.length > 98) {
            this.setState({
                question: value.slice(0, 99)
            });
            this._addNotification('Your question is getting too long.', 'error');
        } else if (name === 'option' && this.state.option.length > 50) {
            this.setState({
                option: value.slice(0, 51)
            });
            this._addNotification('Your  option is getting too long.', 'error');
        } else {
            this.setState({
                [name]: value
            });
    ...
    handleSubmit(event) {
        if (this.state.option.length > 0) {
            ...
            this._addNotification('Your question is too short.', 'error');
            ...
            <div className="NewPoll" style={style.form}>
                <form onSubmit={this.handleSubmit}>
                    ...
                    <div>
                        ...
                        <input value={this.state.question} style={style.input} type="text" name="question" placeholder="What's the crappiest name for a voting app?" onChange={this.handleInputChange}/>
                    </div>
                    <div>
